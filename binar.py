...
def sampletype2np(sample_type: SampleType) -> np.dtype:
    """
    Convert a SampleType (string) to a Numpy dtype
    """
    if sample_type == "f32":
        return np.float32
    if sample_type == "f64":
    ...
    raise ValueError(f'Unrecognised SampleType "{sample_type}"')


def np2sampletype(dtype: np.dtype) -> SampleType:
    """
    Convert a Numpy dtype to a SampleType (string)
    """
    if dtype == np.float32:
        return "f32"
    if dtype == np.float64:
    ...
    if dtype == np.complex128:
        return "c128"
    raise ValueError(f'Unrecognised SampleType "{dtype}"')
...
class SignalFile:
    ...
    def load(self, offset: float = -1.0, length: float = -1.0) -> npt.NDArray:
        dtype = sampletype2np(self.dtype)
        if offset == -1.0 and length == -1.0:
            if self.verbose:
                print(f"- Reading {str(self.path)} {dtype}")
            return np.fromfile(self.path, dtype=dtype)
        ...
        if self.verbose:
            print(
                f"- Reading {str(self.path)} {dtype} {sample_count} samples from {sample_offset}"
            )
        return np.fromfile(
            self.path, dtype=dtype, offset=sample_offset * dtype().nbytes, count=sample_count
        )
    ...
    for channel in channels:
        ...
        if not out_sf.path.exists():
            if iq_data is None:
                iq_data = sf.load()
            out_sf.write(
                channel_power(iq_data, h_channel, channel, sf.samplerate, M_down)
            )
        else:
            if verbose:
...
def cmd_demod(path:Path, channel:float, offset:float, length:float, verbose:bool=False):
    ...
    chan_data = channelise(iq_data, sf.samplerate, channel=channel)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--symbolrate", type=int, default=DEF_SYMBOLRATE, help="GFSK data bitrate (Hz)"
    )
    parser.add_argument(
        "--ntaps",
        type=int,
    ...
    if pargs.cmd == "demod":
        cmd_demod(Path(pargs.srcfile), pargs.channel, offset=pargs.offset, length=pargs.length, verbose=pargs.verbose)
        sys.exit(0)
